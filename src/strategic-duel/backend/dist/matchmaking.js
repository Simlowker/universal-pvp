...
exports.RealTimeMatchmaking = void 0;
const events_1 = require("events");
const ws_1 = require("ws");
class RealTimeMatchmaking extends events_1.EventEmitter {
    constructor() {
        super();
        ...
        this.MATCH_TOLERANCE = 200; // ELO difference tolerance
     ...
     */
    updatePsychProfile(playerId, gameData) {
        const player = this.playerProfiles.get(playerId);
        if (!player)
            return;
        // Analyze decision times
        if (gameData.decisionTimes?.length > 0) {
            const avgTime = gameData.decisionTimes.reduce((a, b) => a + b, 0) / gameData.decisionTimes.length;
     ...
     */
    isGoodMatch(player1, player2, request1, request2) {
        // Same game mode
        if (request1.gameMode !== request2.gameMode)
            return false;
        // Similar bet amounts
        if (request1.betAmount !== request2.betAmount)
            return false;
        // ELO compatibility
        const eloDiff = Math.abs(player1.eloRating - player2.eloRating);
        if (eloDiff > this.MATCH_TOLERANCE)
            return false;
        ...
        if (psychScore < 0.3)
            return false; // Minimum compatibility threshold
        return true;
    }
    /**
     * Calculate psychological compatibility between players
     */
    calculatePsychCompatibility(profile1, profile2) {
     ...
     * Create a new match
     */
    createMatch(player1, player2, request1, request2) {
     ...
     */
    calculateAggression(betPatterns) {
        if (betPatterns.length === 0)
            return 50;
     ...
     */
    notifyPlayer(player, event, data) {
        if (player.websocket && player.websocket.readyState === ws_1.WebSocket.OPEN) {
...
exports.RealTimeMatchmaking = RealTimeMatchmaking;
