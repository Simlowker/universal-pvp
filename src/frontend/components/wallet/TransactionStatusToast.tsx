...
const TransactionStatusToast: React.FC<TransactionStatusToastProps> = ({
  ...
  const getMessage = () => {
    const messages = TRANSACTION_MESSAGES[transaction.type];
    ...
    >
      ...
      <div className="flex items-start gap-3">
        ...
        <div className="flex-shrink-0 mt-0.5">
          {TRANSACTION_ICONS[transaction.type] || <Coins className="h-4 w-4" />}
        </div>
  ...
  useEffect(() => {
    recentTransactions.forEach(tx => {
      if (!visibleTransactions.has(tx.signature)) {
        setVisibleTransactions(prev => new Set([...prev, tx.signature]));
      }
    });
