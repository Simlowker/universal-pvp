...
export const GamePlay: React.FC<GamePlayProps> = ({ className = '' }) => {
  const { 
    isConnected, 
    isConnecting, 
    error, 
  ...
  const [gamePhase, setGamePhase] = useState<'menu' | 'searching' | 'battle' | 'results'>('menu');
  const [searchStartTime, setSearchStartTime] = useState<number>(0);
  const [searchDuration, setSearchDuration] = useState<number>(0);

  // Update game phase based on match state
  useEffect(() => {
    if (gameState.phase === 'completed') {
      setGamePhase('results');
    } else if (isInMatch && gameState.phase === 'battle') {
      setGamePhase('battle');
    } else if (gamePhase === 'searching' && !isInMatch) {
      // Still searching
    } else if (!isInMatch && gamePhase !== 'menu') {
      setGamePhase('menu');
    }
  }, [isInMatch, gameState.phase, gamePhase]);
    ...
    return (
      <div className={`min-h-screen bg-gray-900 flex items-center justify-center p-4 ${className}`}>
        <div className="max-w-md w-full bg-gray-800 rounded-xl p-8 text-center border border-red-600">
          ...
          <button
            onClick={connect}
            className="w-full bg-blue-600 hover:bg-blue-500 text-white font-semibold py-3 px-6 rounded-lg transition-colors duration-200"
          >
            🔄 Retry Connection
          </button>
        </div>
    ...
    return (
      <div className={`min-h-screen bg-gray-900 flex items-center justify-center p-4 ${className}`}>
        <div className="max-w-md w-full bg-gray-800 rounded-xl p-8 text-center">
          ...
          <h2 className="text-2xl font-bold text-white mb-4">Connecting to MagicBlock</h2>
          <p className="text-gray-400">
            Setting up your session key and connecting to the Ephemeral Rollup...
          </p>
        </div>
      </div>
    );
  ...
  if (gamePhase === 'menu') {
    return (
      <div className={`min-h-screen bg-gradient-to-b from-gray-900 via-purple-900/20 to-gray-900 p-4 ${className}`}>
        ...
        <div className="max-w-7xl mx-auto">
          ...
          <div className="mt-6 text-center">
            <div className="inline-flex items-center space-x-4 bg-gray-800 rounded-full px-6 py-2 border border-gray-700">
              ...
              <div className="w-px h-4 bg-gray-600"></div>
              <div className="text-gray-400 text-sm">
                {gameState.actions.length} actions
              </div>
            </div>
          </div>
        </div>
  ...
  if (gamePhase === 'results') {
    return (
      <>
        {/* Battle background (dimmed) */}
        <div className="min-h-screen bg-gray-900 p-4 opacity-50">
          <div className="max-w-7xl mx-auto">
            <div className="grid lg:grid-cols-4 gap-6">
              <div className="lg:col-span-1">
                <PlayerHUD />
